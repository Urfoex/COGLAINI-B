...
void GL_ShapeDrawer::drawOpenGL( btScalar* m, const btCollisionShape* shape, const btVector3& color, int	debugMode, const btVector3& worldBoundsMin, const btVector3& worldBoundsMax ) {
	...
	if( shape->getShapeType() == CUSTOM_CONVEX_SHAPE_TYPE ) {
		std::cout << "CUSTOM_CONVEX_SHAPE_TYPE" << std::endl;
		throw( std::runtime_error( "CUSTOM_CONVEX_SHAPE_TYPE" ) );
...
// 		return;
		...
		if( ( shape->getShapeType() == BOX_SHAPE_PROXYTYPE ) && ( debugMode & btIDebugDraw::DBG_FastWireframe ) ) {
			throw( std::runtime_error( "debugMode & btIDebugDraw::DBG_FastWireframe" ) );
// 			btVector3 org( m[12], m[13], m[14] );
// 			btVector3 dx( m[0], m[1], m[2] );
// 			btVector3 dy( m[4], m[5], m[6] );
// 			btVector3 dz( m[8], m[9], m[10] );
...
// 		glMultMatrixf( m );
		...
		if( shape->getShapeType() == UNIFORM_SCALING_SHAPE_PROXYTYPE ) {
			std::cout << "UNIFORM_SCALING_SHAPE_PROXYTYPE" << std::endl;
...
// 			return;
		...
		if( shape->getShapeType() == COMPOUND_SHAPE_PROXYTYPE ) {
			std::cout << "COMPOUND_SHAPE_PROXYTYPE" << std::endl;
			throw( std::runtime_error( "Removed." ) );
...
// 				std::cout << "m_textureenabled && ( !m_textureinitialized )" << std::endl;
			...
			glMatrixMode( GL_MODELVIEW ); //FIXME: push-pop wrong mat
			checkGLErrorLF( std::cerr );
...
// 				std::cout << "!( debugMode & btIDebugDraw::DBG_DrawWireframe )" << std::endl;
// 				throw(std::runtime_error("Removed."));
...
//
				default: {
						if( shape->isConvex() ) {
// 							std::cout << "shape->isConvex()" << std::endl;
...
// 									std::cout << "hull->numTriangles() > 0" << std::endl;
